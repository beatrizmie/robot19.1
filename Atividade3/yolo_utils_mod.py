...
def draw_labels_and_boxes(img, boxes, confidences, classids, idxs, colors, labels):
    ...
    if len(idxs) > 0:
        ...
        for i in idxs.flatten():
            #Check if is from the class you want
            if labels[classids[i]] == tipo:
...
def infer_image(net, layer_names, height, width, img, colors, labels, FLAGS, 
    ...
    #Only return boxes from the class you want
    boxes_class = []

    for i in range(len(boxes)):
        if labels[classids[i]] == tipo:
            boxes_class.append(boxes[i])

    return img, boxes, boxes_class, confidences, classids, idxs
